...
public class RewriteMe {
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        ...
        return questions.stream().flatMap(s -> s.answers.stream()).anyMatch(s -> s.equals(answerCandidate));
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        ...
        return (int) questions.stream().flatMap(s -> s.answers.stream()).filter(s -> s.equals(answerCandidate)).count();
    }
    ...
    public String getLongestLettercountAnwerInAGivenCategory(Category c) {
       ...
       return questions.stream().filter(s -> s.getCategory().equals(c)).flatMap(s -> s.answers.stream()).reduce("", (u, a) -> {
